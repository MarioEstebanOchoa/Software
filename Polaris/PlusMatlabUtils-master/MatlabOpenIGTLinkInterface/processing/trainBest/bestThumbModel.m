...
name = ['flex_thumb' num2str(type_of_mov) '_time'];
f = fopen(['stats_' name '.txt'],'w');
...
%% Training 
        ...
        all_pred_time = train_prediction_time(input_filt,real_pos, hidden_time, delay, name)';
...
err_time_series_test_t = [];
        for j=init_f_ts:end_f_ts
            if j~=k
                ...
                prediction_time_series = test_time_series(input_filt,real_pos, hidden_time, delay, name);
